...
static qboolean S_MIKMOD_CodecInitialize(void) {
  ...
  MikMod_RegisterDriver(
      &drv_nos); /* only need the "nosound" driver, none else */
  ...
  md_mode &=
      ~DMODE_SIMDMIXER; /* SIMD mixer is buggy when combined with HQMIXER */
...
static qboolean S_MIKMOD_CodecOpenStream(snd_stream_t* stream) {
  ...
  /*	Con_DPrintf("Playing %s (%d chn)\n", priv->module->songname,
   * priv->module->numchn);*/

  return true;
}

static int S_MIKMOD_CodecReadStream(snd_stream_t* stream,
                                    int bytes,
                                    void* buffer) {
  if (!Player_Active())
    return 0;
...
snd_codec_t mikmod_codec = {CODECTYPE_MOD,
                            false,
                            "s3m",
