...
void R_RenderDlights(void) {
  ...
  r_dlightframecount =
      r_framecount + 1;  // because the count hasn't advanced yet for this frame
  glDepthMask(0);
...
/*
=============
RecursiveLightPoint -- johnfitz -- replaced entire function for lit support via
lordhavoc
=============
*/
int RecursiveLightPoint(vec3_t color,
                        mnode_t* node,
                        vec3_t rayorg,
                        vec3_t start,
                        vec3_t end,
...
loc0:
  ...
  if ((back < 0) == (front < 0))
  //		return RecursiveLightPoint (color, node->children[front < 0], rayorg,
  //start, end, maxdist);
  {
  ...
  // go down front side
  if (RecursiveLightPoint(color, node->children[front < 0], rayorg, start, mid,
                          maxdist))
    ...
    for (i = 0; i < node->numsurfaces; i++, surf++) {
      ...
      ds =
          (int)((double)DoublePrecisionDotProduct(mid, surf->texinfo->vecs[0]) +
                surf->texinfo->vecs[0][3]);
      dt =
          (int)((double)DoublePrecisionDotProduct(mid, surf->texinfo->vecs[1]) +
      ...
      if (!surf->samples) {
        // We hit a surface that is flagged as lightmapped, but doesn't have
        // actual lightmap info. Instead of just returning black, we'll keep
        // looking for nearby surfaces that do have valid samples. This fixes
        // occasional pitch-black models in otherwise well-lit areas in DOTM
        // (e.g. mge1m1, mge4m1) caused by overlapping surfaces with mixed
        // lighting data.
      ...
      if (dist < *maxdist) {
        ...
        byte* lightmap;
        int maps, line3, dsfrac = ds & 15, dtfrac = dt & 15, r00 = 0, g00 = 0,
                         b00 = 0, r01 = 0, g01 = 0, b01 = 0, r10 = 0, g10 = 0,
                         b10 = 0, r11 = 0, g11 = 0, b11 = 0;
        float scale;
    ...
    // go down back side
    return RecursiveLightPoint(color, node->children[front >= 0], rayorg, mid,
                               end, maxdist);
  }
}
