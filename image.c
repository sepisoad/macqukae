...
============
*/
qboolean Image_WriteTGA(const char* name,
                        byte* data,
                        int width,
                        int height,
                        int bpp,
  ...
  Sys_mkdir(com_gamedir);  // if we've switched to a nonexistant gamedir, create
                           // it now so we don't crash
...
byte* Image_LoadTGA(FILE* fin, int* width, int* height) {
  ...
  if (targa_header.image_type == 1) {
    if (targa_header.pixel_size != 8 || targa_header.colormap_size != 24 ||
      ...
      Sys_Error("Image_LoadTGA: %s has an %ibit palette", loadfilename,
  ...
  if (targa_header.image_type == 1)  // Uncompressed, paletted images
  {
    ...
    for (i = 0; i < targa_header.colormap_length;
         i++) {  // this palette data is bgr.
...
============
*/
qboolean Image_WriteJPG(const char* name,
                        byte* data,
                        int width,
                        int height,
                        int bpp,
...
qboolean Image_WritePNG(const char* name,
                        byte* data,
                        int width,
                        int height,
                        int bpp,
