...
cvar_t cl_maxpitch = {"cl_maxpitch", "90",
                      CVAR_ARCHIVE};  // johnfitz -- variable pitch clamping
cvar_t cl_minpitch = {"cl_minpitch", "-90",
                      CVAR_ARCHIVE};  // johnfitz -- variable pitch clamping
...
void CL_RelinkEntities(void) {
  ...
  for (i = 1, ent = cl_entities + 1; i < cl.num_entities; i++, ent++) {
    ...
    if (ent->msgtime != cl.mtime[0]) {
      ...
      ent->lerpflags |=
          LERP_RESETMOVE |
          LERP_RESETANIM;  // johnfitz -- next time this entity slot is reused,
                           // the lerp will need to be reset
      continue;
    }
