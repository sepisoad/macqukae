...
static void S_ApplyFilter(filter_t* filter, int* data, int stride, int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    data[i * stride] =
        (val[0] + val[1] + val[2] + val[3]) * (32768.0 * 256.0 * 4.0);
...
==============
*/
static void S_LowpassFilter(int* data,
                            int stride,
                            int count,
                            filter_t* memory) {
  int M;
...
static void SND_PaintChannelFrom8(channel_t* ch,
                                  sfxcache_t* sc,
                                  int endtime,
                                  int paintbufferstart);
static void SND_PaintChannelFrom16(channel_t* ch,
                                   sfxcache_t* sc,
                                   int endtime,
                                   int paintbufferstart);
...
void S_PaintChannels(int endtime) {
  ...
  while (paintedtime < endtime) {
    ...
    for (i = 0; i < end - paintedtime; i++) {
      paintbuffer[i].left =
          CLAMP(-32768 * 256, paintbuffer[i].left, 32767 * 256) / 2;
      paintbuffer[i].right =
          CLAMP(-32768 * 256, paintbuffer[i].right, 32767 * 256) / 2;
    }
    ...
    // paint in the music
    if (s_rawend >= paintedtime) {  // copy from the streaming sound source
      int s;
      int stop;
