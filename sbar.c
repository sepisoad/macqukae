...
static int hipweapons[4] = {HIT_LASER_CANNON_BIT, HIT_MJOLNIR_BIT, 4,
...
void Sbar_DrawPicAlpha(int x, int y, qpic_t* pic, float alpha) {
  ...
  Draw_Pic(x, y + 24, pic);
  glColor4f(1, 1, 1,
            1);  // ericw -- changed from glColor3f to work around intel 855 bug
                 // with "r_oldwater 0" and "scr_sbaralpha 0"
  glDisable(GL_BLEND);
...
void Sbar_DrawNum(int x, int y, int num, int digits, int color) {
  ...
  while (*ptr) {
    ...
    Sbar_DrawPic(
        x, y, sb_nums[color][frame]);  // johnfitz -- DrawTransPic is obsolete
    x += 24;
    ptr++;
  }
}
...
void Sbar_SoloScoreboard(void) {
  ...
  sprintf(str, "Secrets: %i/%i", cl.stats[STAT_SECRETS],
...
void Sbar_DrawInventory(void) {
  ...
  if (rogue) {
    ...
    Sbar_DrawPicAlpha(0, -24, sb_ibar,
                      scr_sbaralpha.value);  // johnfitz -- scr_sbaralpha
  }
...
void Sbar_DrawFace(void) {
  ...
  if (rogue && (cl.maxclients != 1) && (teamplay.value > 3) &&
    ...
    Draw_Fill(xofs, /*vid.height-*/ 24 + 3, 22, 9, top,
              1);  // johnfitz -- sbar coords are now relative
    Draw_Fill(xofs, /*vid.height-*/ 24 + 12, 22, 9, bottom,
              1);  // johnfitz -- sbar coords are now relative
...
void Sbar_DeathmatchOverlay(void) {
  ...
  x = 80;  // johnfitz -- simplified becuase some positioning is handled
           // elsewhere
  y = 40;
  for (i = 0; i < l; i++) {
    k = fragsort[i];
...
#if 0
{
    ...
    // draw name
    M_Print(x + 64, y, s->name);  // johnfitz -- was Draw_String, changed for
                                  // stretched overlays
...
void Sbar_IntermissionOverlay(void) {
  ...
  q_snprintf(time, sizeof(time), "%d:%02d", cl.completed_time / 60,
             cl.completed_time % 60);
  q_snprintf(secrets, sizeof(secrets), "%d/%2d", cl.stats[STAT_SECRETS],
             cl.stats[STAT_TOTALSECRETS]);
  q_snprintf(monsters, sizeof(monsters), "%d/%2d", cl.stats[STAT_MONSTERS],
             cl.stats[STAT_TOTALMONSTERS]);
