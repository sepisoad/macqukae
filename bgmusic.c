...
static void BGM_Play_noext(const char* filename, unsigned int allowed_types) {
  ...
  while (handler) {
    ...
    q_snprintf(tmp, sizeof(tmp), "%s/%s.%s", handler->dir, filename,
               handler->ext);
...
void BGM_PlayCDtrack(byte track, qboolean looping) {
  ...
  while (handler) {
    ...
    //		goto _next;
    q_snprintf(tmp, sizeof(tmp), "%s/track%02d.%s", MUSIC_DIRNAME, (int)track,
               handler->ext);
...
static void BGM_UpdateStream(void) {
  ...
  while (s_rawend < paintedtime + MAX_RAW_SAMPLES) {
    ...
    /* our max buffer size */
    fileBytes =
        fileSamples * (bgmstream->info.width * bgmstream->info.channels);
    if (fileBytes > (int)sizeof(raw)) {
      fileBytes = (int)sizeof(raw);
      fileSamples =
          fileBytes / (bgmstream->info.width * bgmstream->info.channels);
    }
