...
void CL_ParseUpdate(int bits) {
  ...
  // johnfitz -- lerping
  if (ent->msgtime + 0.2 <
      cl.mtime[0])  // more than 0.2 seconds since the last message (most
...
void CL_ParseClientdata(void) {
  ...
  bits = (unsigned short)MSG_ReadShort();  // johnfitz -- read bits here isntead
      ...
      case svc_signonnum:
        ...
        if (i == 2) {
          if (cl.num_statics > 128)
            Con_DWarning(
                "%i static entities exceeds standard limit of 128 (max = "
                "%d).\n",
