...
================
*/
static SDL_DisplayMode* VID_SDL2_GetDisplayMode(int width,
                                                int height,
                                                int refreshrate,
                                                int bpp) {
...
================
*/
static qboolean VID_ValidMode(int width,
                              int height,
                              int refreshrate,
                              int bpp,
                              qboolean fullscreen) {
...
================
*/
static qboolean VID_SetMode(int width,
                            int height,
                            int refreshrate,
                            int bpp,
                            qboolean fullscreen) {
...
static void GL_CheckExtensions(void) {
  ...
  else if (host_bigendian) /** FIXME: See
                              https://github.com/sezero/quakespasm/issues/114
                              **/
...
/*
=================
VID_DescribeModes_f -- johnfitz -- changed formatting, and added refresh rates
after each mode.
=================
*/
...
void VID_Toggle(void) {
  ...
  // changing the window size on SDL2/WinXP and we weren't set up to handle it.
  // --ericw
  //
  // TODO: Clear out the dead code, reinstate the fast path using
  // SDL_SetWindowFullscreen inside VID_SetMode, check window size to fix WinXP
...
static void VID_Menu_ChooseNextMode(int dir) {
  ...
  if (vid_menu_nummodes) {
    ...
    if (i == vid_menu_nummodes)  // can't find it in list, so it must be a
                                 // custom windowed res
    {
      i = 0;
    } else {
