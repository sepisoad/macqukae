...
static char* PF_VarString(int first) {
  ...
  if (LOC_HasPlaceholders(format)) {
    ...
    for (i = first; i < pr_argc; i++) {
      s = q_strlcat(out, LOC_GetString(G_STRING(OFS_PARM0 + i * 3)),
                    sizeof(out));
      if (s >= sizeof(out)) {
  ...
  if (s > 255) {
    if (!dev_overflows.varstring ||
      ...
      Con_DWarning(
          "PF_VarString: %i characters exceeds standard limit of 255 (max = "
          "%d).\n",
...
static void SetMinMaxSize(edict_t* e,
                          float* minvec,
                          float* maxvec,
                          qboolean rotate) {
  float* angles;
...
static builtin_t pr_builtin[] = {
    ...
    PF_Fixme,  // void draw_point (vector point, float colormap, float lifetime,
               // float depthtest) = #81
    PF_Fixme,  // void draw_line (vector start, vector end, float colormap,
               // float lifetime, float depthtest) = #82
    PF_Fixme,  // void draw_arrow (vector start, vector end, float colormap,
               // float size, float lifetime, float depthtest) = #83
    PF_Fixme,  // void draw_ray (vector start, vector direction, float length,
               // float colormap, float size, float lifetime, float depthtest) =
               // #84
    PF_Fixme,  // void draw_circle (vector origin, float radius, float colormap,
               // float lifetime, float depthtest) = #85
    PF_Fixme,  // void draw_bounds (vector min, vector max, float colormap,
               // float lifetime, float depthtest) = #86
    PF_Fixme,  // void draw_worldtext (string s, vector origin, float size,
               // float lifetime, float depthtest) = #87
    PF_Fixme,  // void draw_sphere (vector origin, float radius, float colormap,
               // float lifetime, float depthtest) = #88
    PF_Fixme,  // void draw_cylinder (vector origin, float halfHeight, float
               // radius, float colormap, float lifetime, float depthtest) = #89
