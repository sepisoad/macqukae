...
qboolean SV_movestep(edict_t* ent, vec3_t move, qboolean relink) {
  ...
  if ((int)ent->v.flags & (FL_SWIM | FL_FLY)) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      trace =
          SV_Move(ent->v.origin, ent->v.mins, ent->v.maxs, neworg, false, ent);
  ...
  if (!SV_CheckBottom(ent)) {
    if ((int)ent->v.flags &
        FL_PARTIALGROUND) {  // entity had floor mostly pulled out from
                             // underneath it
...
void SV_NewChaseDir(edict_t* actor, edict_t* enemy, float dist) {
  ...
  if (((rand() & 3) & 1) ||
      abs((int)deltay) >
          abs((int)deltax))  // ericw -- explicit int cast to suppress clang
                             // suggestion to use fabsf
  {
    tdir = d[1];
    d[1] = d[2];
    d[2] = tdir;
