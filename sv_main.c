...
static qboolean SV_IsLocalClient(client_t* client) {
  return Q_strcmp(NET_QSocketGetAddressString(client->netconnection),
                  "LOCAL") == 0;
}
...
=============
*/
qboolean SV_VisibleToClient(edict_t* client,
                            edict_t* test,
                            qmodel_t* worldmodel) {
  byte* pvs;
  vec3_t org;
...
void SV_WriteClientdataToMessage(edict_t* ent, sizebuf_t* msg) {
  ...
  if (bits & SU_WEAPONALPHA)
    MSG_WriteByte(msg,
                  ent->alpha);  // for now, weaponalpha = client entity alpha
  // johnfitz
}
    ...
    if (host_client->spawned) {
      ...
      if (host_client->sendsignon == PRESPAWN_SIGNONBUFS) {
        ...
        while (host_client->signonidx < sv.num_signon_buffers) {
          ...
          if (host_client->message.cursize + signon->cursize >
          ...
          // the datagram buffer for clients using a lower limit (e.g. 32000 in
          // QS)
          if (!local)
            break;
        }
...
void SV_CreateBaseline(void) {
  ...
  for (entnum = 0; entnum < sv.num_edicts; entnum++) {
    ...
    bits = 0;
    if (sv.protocol ==
        PROTOCOL_NETQUAKE)  // still want to send baseline in PROTOCOL_NETQUAKE,
...
void SV_SpawnServer(const char* server) {
  ...
  if (signonsize > 64000 - 2)
    Con_DWarning("%i byte signon buffer exceeds QS limit of 63998.\n",
           ...
           8000 - 2)  // max size that will fit into 8000-sized client->message
