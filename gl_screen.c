...
static void SCR_CalcRefdef(void) {
  ...
  r_refdef.vrect.width =
      q_max(glwidth * size, 96.0f);  // no smaller than 96, for icons
  r_refdef.vrect.height =
      q_min((int)(glheight * size), glheight - sb_lines);  // make room for sbar
...
void SCR_SetUpToDrawConsole(void) {
  ...
  if (con_forcedup) {
    scr_conlines =
        glheight;  // full screen //johnfitz -- glheight instead of vid.height
  ...
  if (scr_conlines < scr_con_current) {
    ...
    scr_con_current -= conspeed * (glheight / 600.0) * host_frametime /
                       timescale;  // johnfitz -- timescale
    if (scr_conlines > scr_con_current)
      scr_con_current = scr_conlines;
...
void SCR_ScreenShot_f(void) {
  ...
  for (i = 0; i < 10000; i++) {
    q_snprintf(imagename, sizeof(imagename), "spasm%04i.%s", i,
               ext);  // "fitz%04i.tga"
  ...
  if (!q_strncasecmp(ext, "png", sizeof(ext)))
    ...
    ok = Image_WriteJPG(imagename, buffer, glwidth, glheight, 24, quality,
                        false);
  else
    ok = false;
