...
=================
*/
static void TexMgr_LoadMiptexPalette(byte* in,
                                     byte* out,
                                     int numcolors,
                                     unsigned flags) {
...
================
*/
static unsigned* TexMgr_ResampleTexture(unsigned* in,
                                        int inwidth,
                                        int inheight,
                                        qboolean alpha) {
...
static void TexMgr_LoadImage32(gltexture_t* glt, unsigned* data) {
  ...
  if (glt->flags & TEXPREF_MIPMAP &&
      !(glt->flags &
        TEXPREF_WARPIMAGE))  // warp image mipmaps are generated later
  {
    mipwidth = glt->width;
...
================
*/
static void TexMgr_LoadImage8(gltexture_t* glt,
                              byte* data,
                              unsigned int* usepal) {
  extern cvar_t gl_fullbrights;
  ...
  if (strstr(glt->name, "shot1sid") && glt->width == 32 && glt->height == 32 &&
    ...
    // white. They are invisible in software, but look really ugly in GL. So we
    // just copy 32 pixels from the bottom to make it look nice.
...
static void TexMgr_LoadLightmap(gltexture_t* glt, byte* data) {
  ...
  // upload it
  GL_Bind(glt);
  glTexImage2D(GL_TEXTURE_2D, 0, internalfmt, glt->width, glt->height, 0,
               gl_lightmap_format, type, data);
...
================
*/
gltexture_t* TexMgr_LoadImage(qmodel_t* owner,
                              const char* name,
                              int width,
                              int height,
...
void TexMgr_ReloadImages(void) {
  ...
  // TexMgr to free the texture. If this frees 'glt' in the loop below, the
  // active_gltextures list gets corrupted. A test case is jam3_tronyn.bsp with
  // -heapsize 65536, and do several mode switches/fullscreen toggles 2015-09-04
  // -- Cache_Flush workaround was causing issues
  // (http://sourceforge.net/p/quakespasm/bugs/10/) switching to a boolean flag.
