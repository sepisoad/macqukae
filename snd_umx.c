...
static int read_export(fshandle_t* f,
...
static int read_typname(fshandle_t* f,
                        const struct upkg_hdr* hdr,
                        int idx,
                        char* out) {
  int i, s;
...
static int probe_umx(fshandle_t* f,
                     const struct upkg_hdr* hdr,
                     int32_t* ofs,
                     int32_t* objsize) {
  int i, idx, t;
  ...
  if (hdr->name_offset >= fsiz || hdr->export_offset >= fsiz ||
...
static int32_t probe_header(fshandle_t* f, struct upkg_hdr* hdr) {
  ...
  if (hdr->name_count < 0 || hdr->export_count < 0 || hdr->import_count < 0 ||
      hdr->name_offset < 36 || hdr->export_offset < 36 ||
...
static int S_UMX_CodecReadStream(snd_stream_t* stream,
                                 int bytes,
                                 void* buffer) {
  return -1;
}
...
snd_codec_t umx_codec = {CODECTYPE_UMX,
