...
/*
================
Con_Quakebar -- johnfitz -- returns a bar of the desired length, but never wider
than the console
...
============
*/
const char* FindCompletion(const char* partial,
                           filelist_item_t* filelist,
                           int* nummatches_out) {
...
void Con_TabComplete(void) {
  ...
  *c = '\0';              // now copy all of this into edit line
  ...
  if (key_linepos < MAXCMDLINE - 1 && key_lines[edit_line][key_linepos] == 0 &&
    ...
    // bash-style display behavior changes, causes weirdness when completing the
    // arguments for the changelevel command. the line below "fixes" it,
...
void Con_DrawInput(void) {
  ...
  for (i = 0; key_lines[edit_line][i + ofs] && i < con_linewidth; i++)
    Draw_Character((i + 1) << 3, vid.conheight - 16,
...
void Con_DrawConsole(int lines, qboolean drawinput) {
  ...
  for (x = 0; x < (int)strlen(ver); x++)
    Draw_Character((con_linewidth - strlen(ver) + x + 2) << 3, y,
                   ver[x] /*+ 128*/);
}
...
void LOG_Init(quakeparms_t* parms) {
  ...
  q_snprintf(logfilename, sizeof(logfilename), "%s/qconsole.log",
             parms->basedir);
