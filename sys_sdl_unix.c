...
static qboolean Sys_GetUserdirArgs(int argc,
                                   char** argv,
                                   char* dst,
                                   size_t dstsize) {
  int i = 1;
...
static void Sys_GetUserdir(int argc, char** argv, char* dst, size_t dstsize) {
  ...
  result = find_directory(B_USER_NONPACKAGED_DATA_DIRECTORY, volume, false,
...
static char* OSX_StripAppBundle(
    char* dir) { /* based on the ioquake3 project at icculus.org. */
...
void Sys_SendKeyEvents(void) {
  IN_Commands();  // ericw -- allow joysticks to add keys so they can be used to
