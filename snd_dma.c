  ...
  for (ch_idx = NUM_AMBIENTS; ch_idx < NUM_AMBIENTS + MAX_DYNAMIC_CHANNELS;
    ...
    if (entchannel != 0  // channel 0 never overrides
        ...
        (snd_channels[ch_idx].entchannel == entchannel ||
         entchannel == -1)) {  // always override sound from same entity
      first_to_die = ch_idx;
      break;
    }
...
void S_RawSamples(int samples,
  ...
  if (channels == 2 && width == 2) {
    ...
    for (i = 0;; i++) {
      ...
      //	s_rawsamples [dst].right = ((signed char *) data)[src * 2 + 1] *
      //intVolume;
...
static void GetSoundtime(void) {
  ...
  if (samplepos < oldsamplepos) {
    ...
    if (paintedtime >
        0x40000000) {  // time to chop things off to avoid 32 bit limits
      buffers = 0;
...
static void S_SoundList(void) {
  ...
  for (sfx = known_sfx, i = 0; i < num_sfx; i++, sfx++) {
    ...
    Con_SafePrintf("(%2db) %6i : %s\n", sc->width * 8, size,
                   sfx->name);  // johnfitz -- was Con_Printf
  }
  Con_Printf("%i sounds, %i bytes\n", num_sfx,
             total);  // johnfitz -- added count
}
