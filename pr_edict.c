...
int pr_effects_mask;  // only enable 2021 rerelease quad/penta dlights when
                      // applicable
...
static const char* PR_ValueString(int type, eval_t* val) {
  ...
  switch (type) {
    case ev_string:
      ...
      break;
    case ev_entity:
      q_snprintf(line, sizeof(line), "entity %i",
    ...
    case ev_float:
      ...
      break;
    case ev_vector:
      q_snprintf(line, sizeof(line), "'%5.1f %5.1f %5.1f'", val->vector[0],
...
static const char* PR_UglyValueString(int type, eval_t* val) {
  ...
  switch (type) {
    ...
    case ev_float:
      ...
      break;
    case ev_vector:
      q_snprintf(line, sizeof(line), "%f %f %f", val->vector[0], val->vector[1],
                 val->vector[2]);
      break;
    default:
      q_snprintf(line, sizeof(line), "bad type %i", type);
...
const char* PR_GlobalString(int ofs) {
  ...
  if (!def)
    ...
    q_snprintf(line, sizeof(line), "%i(%s)%s", ofs, PR_GetString(def->s_name),
               s);
  }
...
const char* ED_ParseEdict(const char* data, edict_t* ent) {
  ...
  while (1) {
    ...
    data = COM_ParseEx(data,
                       !strcmp(keyname, "wad") ? CPE_ALLOWTRUNC : CPE_NOTRUNC);
    if (!data)
      Host_Error("ED_ParseEntity: EOF without closing brace");
...
static qboolean PR_HasGlobal(const char* name, float value) {
  ...
  return g && (g->type & ~DEF_SAVEGLOBAL) == ev_float &&
         G_FLOAT(g->ofs) == value;
}
...
static int PR_FindSupportedEffects(void) {
  ...
  return isqex ? -1
               : -1 & ~(EF_QEX_QUADLIGHT | EF_QEX_PENTALIGHT |
                        EF_QEX_CANDLELIGHT);
}
...
static const exbuiltin_t exbuiltins[] = {
    /* Update-1 adds the following builtins with new ids. Patch them to use old
     * indices.
    ...
    /* Update-3 changes its unique builtins to be looked up by name instead of
     * builtin numbers, to avoid conflict with other engines. Patch them to use
     * our indices.
...
static void PR_AllocStringSlots(void) {
  ...
  Con_DPrintf2("PR_AllocStringSlots: realloc'ing for %d slots\n",
               pr_maxknownstrings);
  pr_knownstrings = (const char**)Z_Realloc((void*)pr_knownstrings,
