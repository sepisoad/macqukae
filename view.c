...
void V_PolyBlend(void) {
  ...
  glColor4fv(
      v_blend);  // inside glBegin / glEnd to workaround an AMD driver bug
...
void V_CalcRefdef(void) {
  ...
  // from getting covered by sbar) MarkV -- restored this with r_viewmodel_quake
  // cvar
  ...
  if (v_gunkick.value == 2)  // lerped kick
  {
    for (i = 0; i < 3; i++)
      if (punch[i] != v_punchangles[0][i]) {
        ...
        delta =
            (v_punchangles[0][i] - v_punchangles[1][i]) * host_frametime * 10;
