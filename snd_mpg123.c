...
static qboolean S_MP3_CodecOpenStream(snd_stream_t* stream) {
  ...
  if (mpg123_replace_reader_handle(priv->handle, mp3_read, mp3_seek, NULL) !=
          MPG123_OK ||
...
static int S_MP3_CodecReadStream(snd_stream_t* stream,
                                 int bytes,
                                 void* buffer) {
  mp3_priv_t* priv = (mp3_priv_t*)stream->priv;
  size_t bytes_read = 0;
  int res = mpg123_read(priv->handle, (unsigned char*)buffer, (size_t)bytes,
                        &bytes_read);
  switch (res) {
    case MPG123_DONE:
      Con_DPrintf("mp3 EOF\n");
...
snd_codec_t mp3_codec = {CODECTYPE_MP3,
                         false,
                         "mp3",
