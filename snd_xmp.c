...
static unsigned long xmp_fread(void* dest,
                               unsigned long len,
                               unsigned long nmemb,
                               void* f) {
...
static int S_XMP_CodecReadStream(snd_stream_t* stream,
                                 int bytes,
                                 void* buffer) {
  int r;
...
static int S_XMP_CodecRewindStream(snd_stream_t* stream) {
  ...
  if (ret < 0)
    return ret;
  xmp_play_buffer((xmp_context)stream->priv, NULL, 0,
                  0); /* reset internal state */
  return 0;
}

snd_codec_t xmp_codec = {CODECTYPE_MOD,
