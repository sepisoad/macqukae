...
void Vec_Append(void** pvec,
  ...
  memcpy((byte*)*pvec + VEC_HEADER(*pvec).size * element_size, data,
         count * element_size);
...
void MSG_WriteAngle(sizebuf_t* sb, float f, unsigned int flags) {
  if (flags & PRFL_FLOATANGLE)
    ...
    MSG_WriteByte(sb, Q_rint(f * 256.0 / 360.0) &
...
void* SZ_GetSpace(sizebuf_t* buf, int length) {
  ...
  if (buf->cursize + length > buf->maxsize) {
    if (!buf->allowoverflow)
      Host_Error(
          "SZ_GetSpace: overflow without allowoverflow set");  // ericw -- made
                                                               // Host_Error to
                                                               // be less
                                                               // annoying
...
===========
*/
static int COM_FindFile(const char* filename,
                        int* handle,
                        FILE** file,
...
void COM_LoadCacheFile(const char* path,
...
byte* COM_LoadStackFile(const char* path,
                        void* buffer,
                        int bufsize,
                        unsigned int* path_id) {
  byte* buf;
...
static pack_t* COM_LoadPackFile(const char* packfile) {
  ...
  if (Sys_FileRead(packhandle, &header, sizeof(header)) !=
          (int)sizeof(header) ||
      header.id[0] != 'P' || header.id[1] != 'A' || header.id[2] != 'C' ||
      header.id[3] != 'K')
...
static size_t mz_zip_file_read_func(void* opaque,
                                    mz_uint64 ofs,
                                    void* buf,
                                    size_t n) {
...
void LOC_LoadFile(const char* file) {
  ...
  if (!rw) {
    ...
    if (!mz_zip_reader_init(&archive, sz, 0))
      goto fail;
    localization.text =
        (char*)mz_zip_reader_extract_file_to_heap(&archive, file, &size, 0);
    if (!localization.text)
      goto fail;
    ...
    if (line[0] == '/') {
      ...
      *key_end = 0;
      ...
      while (value_src != cursor) {
        if (*value_src == '\\' && value_src + 1 != cursor) {
          ...
          switch (c) {
            ...
            default:
              Con_Printf(
                  "LOC_LoadFile: unrecognized escape sequence \\%c on line "
                  "%d\n",
                  c, lineno);
              *value_dst++ = c;
              break;
      ...
      if (localization.numentries == localization.maxnumentries) {
        ...
        localization.entries = (locentry_t*)realloc(
            localization.entries,
            sizeof(*localization.entries) * localization.maxnumentries);
      }
