...
#define FLAC__stream_decoder_process_until_end_of_metadata \
...
#define FLAC__STREAM_DECODER_READ_STATUS_ABORT \
...
#define FLAC__STREAM_DECODER_SEEK_STATUS_ERROR \
...
/* CALLBACK FUNCTIONS: */
static void flac_error_func(const FLAC__StreamDecoder* decoder,
...
static FLAC__StreamDecoderReadStatus flac_read_func(
...
static FLAC__StreamDecoderSeekStatus flac_seek_func(
    const FLAC__StreamDecoder* decoder,
...
static FLAC__StreamDecoderTellStatus flac_tell_func(
    const FLAC__StreamDecoder* decoder,
    FLAC__uint64* absolute_byte_offset,
    void* client_data) {
  flacfile_t* ff = (flacfile_t*)client_data;
...
static FLAC__StreamDecoderLengthStatus flac_length_func(
    const FLAC__StreamDecoder* decoder,
...
static FLAC__bool flac_eof_func(const FLAC__StreamDecoder* decoder,
                                void* client_data) {
  flacfile_t* ff = (flacfile_t*)client_data;
  if (FS_feof(ff->file))
    return true;
...
static FLAC__StreamDecoderWriteStatus flac_write_func(
    const FLAC__StreamDecoder* decoder,
...
static void flac_meta_func(const FLAC__StreamDecoder* decoder,
...
#else
  rc = FLAC__stream_decoder_init_stream(
      ff->decoder, flac_read_func, flac_seek_func, flac_tell_func,
      flac_length_func, flac_eof_func, flac_write_func, flac_meta_func,
      flac_error_func, ff);
#endif
...
snd_codec_t flac_codec = {CODECTYPE_FLAC,
