...
sys_socket_t WINS_OpenSocket(int port) {
  ...
  if ((newsocket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) ==
      INVALID_SOCKET) {
    err = SOCKETERRNO;
  ...
  if (tcpipAvailable) {
    ...
    Sys_Error("Unable to bind to %s (%s)",
              WINS_AddrToString((struct qsockaddr*)&address), socketerror(err));
...
int WINS_Read(sys_socket_t socketid,
              byte* buf,
              int len,
  ...
  ret =
      recvfrom(socketid, (char*)buf, len, 0, (struct sockaddr*)addr, &addrlen);
  if (ret == SOCKET_ERROR) {
    int err = SOCKETERRNO;
...
int WINS_Write(sys_socket_t socketid,
               byte* buf,
               int len,
               struct qsockaddr* addr) {
  int ret;
