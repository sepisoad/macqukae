...
double key_blinktime;  // johnfitz -- fudge cursor blinking to make it easier to
                       // spot in certain cases
...
void Key_Console(int key) {
  ...
  switch (key) {
    ...
    case K_KP_ENTER:
      ...
      key_lines[edit_line][1] = 0;  // johnfitz -- otherwise old history items
                                    // show up in the new edit line
      key_linepos = 1;
      if (cls.state == ca_disconnected)
        SCR_UpdateScreen();  // force an update, because the command may take
                             // some time
      return;
    ...
    case K_RIGHTARROW:
      ...
      if ((int)len == key_linepos) {
        ...
        workline += key_linepos;
        *workline = key_lines[(edit_line + (CMDLINES - 1)) & (CMDLINES - 1)]
                             [key_linepos];
        workline[1] = 0;
        key_linepos++;
      } else {
...
Called by the system between frames for both key up and key down events
Should NOT be called during an interrupt!
keycode parameter should have the key's actual keycode using the current
keyboard layout, not necessarily the US-keyboard-based scancode. Pass 0 if not
applicable.
===================
*/
...
qboolean Key_TextEntry(void) {
  if (key_inputgrab.active) {
    ...
    // accept controller input, so we don't want an onscreen keyboard for this
    // case.
    return false;
  }
  ...
  switch (key_dest) {
    ...
    case key_menu:
      return M_TextEntry();
    case key_game:
      // Don't return true even during con_forcedup, because that happens while
      // starting a game and we don't to trigger text input (and the onscreen
      // keyboard on some devices) during this.
      return false;
    case key_console:
      return true;
    default:
