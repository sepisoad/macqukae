...
static qboolean is_id3v2(const unsigned char* data, size_t length) {
  ...
  if (data[6] >= 0x80 || data[7] >= 0x80 || data[8] >= 0x80 ||
      data[9] >= 0x80) {
    return false;
  }
  return true;
...
static long get_id3v2_len(const unsigned char* data, long length) {
  ...
  long size =
      (long)((data[6] << 21) + (data[7] << 14) + (data[8] << 7) + data[9]);
  size += 10; /* header size */
...
static long get_ape_len(const unsigned char* data) {
  ...
  long size =
      (long)((data[15] << 24) | (data[14] << 16) | (data[13] << 8) | data[12]);
  version = (unsigned)((data[11] << 24) | (data[10] << 16) | (data[9] << 8) |
                       data[8]);
  flags = (unsigned)((data[23] << 24) | (data[22] << 16) | (data[21] << 8) |
                     data[20]);
...
static inline qboolean verify_lyrics3v2(const unsigned char* data,
                                        long length) {
  if (length < 11)
    return false;
...
static qboolean is_musicmatch(const unsigned char* data, long length) {
  /* From docs/musicmatch.txt in id3lib:
     https://sourceforge.net/projects/id3lib/ Overall tag structure:
  ...
  if (!q_isdigit(data[32]) || data[33] != '.' || !q_isdigit(data[34]) ||
      !q_isdigit(data[35])) {
    return false;
  }
...
static long get_musicmatch_len(snd_stream_t* stream) {
  ...
  version_ofs =
      (int)((buf[15] << 24) | (buf[14] << 16) | (buf[13] << 8) | buf[12]);
...
/* [0..9]: sync string, [30..255]: 0x20 */
  ...
  if (j + 12 != version_ofs - imgext_ofs)
    return -1;
