...
=============
*/
void GL_DrawAliasFrame_GLSL(aliashdr_t* paliashdr,
                            lerpdata_t lerpdata,
                            gltexture_t* tx,
                            gltexture_t* fb) {
  float blend;
  ...
  if (r_lerpmodels.value &&
    ...
    if (e->lerpflags & LERP_FINISH && numposes == 1)
      lerpdata->blend = CLAMP(
          0.0f,
          (float)(cl.time - e->lerpstart) / (e->lerpfinish - e->lerpstart),
          1.0f);
    else
      lerpdata->blend =
          CLAMP(0.0f, (float)(cl.time - e->lerpstart) / e->lerptime, 1.0f);
...
void R_DrawAliasModel(entity_t* e) {
  ...
  glScalef(paliashdr->scale[0], paliashdr->scale[1] * fovscale,
